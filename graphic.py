    ...
    def key_check_menu(self):
        ...
        for self.EVENT in pygame.event.get():
            ...
            if self.EVENT.type == pygame.MOUSEBUTTONDOWN:
                if self.get_mouse_for_button(self.MENU_BUTTON_1_POINT[1]):
                    self.LEVELS = True
                    self.MENU = False
                    self.create_map()
                if self.get_mouse_for_button(self.MENU_BUTTON_2_POINT[1]):
                    self.EDITOR = True
                    self.MENU = False
                    self.create_map()
    ...
    def key_check_editor(self):
        ...
        for self.EVENT in pygame.event.get():
            ...
            if self.EVENT.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.CELL_MENU:
                    # земля
                    if self.get_mouse_for_button(self.CELL_MENU_POINTS[1]):
                        self.map.scheme[global_names.TEMP_CELL[1]][
                            global_names.TEMP_CELL[0]] = self.grass
                    # дорога
                    elif self.get_mouse_for_button(self.CELL_MENU_POINTS[2]):
                        self.map.scheme[global_names.TEMP_CELL[1]][
                            global_names.TEMP_CELL[0]] = self.road
                    elif self.get_mouse_for_button(self.CELL_MENU_POINTS[3]):
                        self.map.scheme[global_names.TEMP_CELL[1]][
                            global_names.TEMP_CELL[0]] = self.spawner
                    elif self.get_mouse_for_button(self.CELL_MENU_POINTS[4]):
                        self.map.scheme[global_names.TEMP_CELL[1]][
        ...
        for self.EVENT in pygame.event.get():
            ...
            if self.EVENT.type == pygame.MOUSEBUTTONDOWN:
                if self.PLAY:
                    ...
                    # стрелка влево
                    if self.get_mouse_for_button(self.LEVELS_ARROWS_POINTS[2]):
                    ...
                    # стрелка вправо
                        ...
                        if global_names.TEMP_ID in range(
                            ...
                            print("p")
                    # кнопка начала игры
                    elif self.get_mouse_for_button(self.LEVELS_PLAY_POINTS[1]):
                        self.PLAY = True
